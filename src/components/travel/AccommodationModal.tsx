  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const initial: Partial<Accommodation>[] = Array.from({ length: totalDays }).map((_, i) => {
        ...
        return found
          ? { ...found }
          : { travelId, dayIndex, name: '', address: '', description: '' };
      });
      setAccList(initial);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900/50">
                  ...
                  <th className="p-3 border-b dark:border-gray-700 font-semibold text-sm w-1/3">주소</th>
                  <th className="p-3 border-b dark:border-gray-700 font-semibold text-sm">비고</th>
                </tr>
              </thead>
              <tbody>
                {accList.map((acc, index) => (
                  <tr key={index} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="p-2 text-center font-medium text-sm text-amber-600 dark:text-amber-500">
                      {acc.dayIndex}일차
                    </td>
                    <td className="p-2">
                      <input type="text" value={acc.name || ''} onChange={(e) => handleChange(index, 'name', e.target.value)} placeholder="숙소명 입력" className="w-full p-2 text-sm border rounded-lg bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 outline-none focus:border-amber-500 transition-colors" />
                    </td>
                    <td className="p-2">
                      <input type="text" value={acc.address || ''} onChange={(e) => handleChange(index, 'address', e.target.value)} placeholder="주소 입력 (선택)" className="w-full p-2 text-sm border rounded-lg bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 outline-none focus:border-amber-500 transition-colors" />
                    </td>
                    <td className="p-2">
                      <input type="text" value={acc.description || ''} onChange={(e) => handleChange(index, 'description', e.target.value)} placeholder="비고 입력 (선택)" className="w-full p-2 text-sm border rounded-lg bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 outline-none focus:border-amber-500 transition-colors" />
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700 flex justify-end shrink-0 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl">
          <button onClick={onClose} className="px-4 py-2 mr-2 bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
            취소
          </button>
          <button onClick={handleSave} className="px-4 py-2 bg-amber-600 text-white rounded-lg text-sm font-medium hover:bg-amber-700 transition-colors flex items-center gap-2 shadow-sm">
            <Save size={16} /> 일괄 저장
          </button>
        </div>
      </div>
